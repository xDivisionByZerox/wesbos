...
![directory with package.json module is created](./1278.png)
...
```sh
npm i @faker-js date-fns await-to-js lodash axios
```

Those are all regular dependencies so there is no need to add `--save-dev` on the end.

![all dependencies in package.json file](./1279.png)
...
![run npm start and open page will come up with this error](./1280.png)
...
![unexpected token < error in console](./1281.png)
...
Next we will look at [`faker`](https://fakerjs.dev/).

![npm package faker](./1283.png)
...
![import faker and call different methods to generate things like fake names and emails](./1284.png)

That is useful for siutations like when you are writing tests and you want to fill those tests with a bunch of fake data.

Let's start by importing it `import { faker } from '@faker-js/faker';`.
...
That is the older node.js syntax, and it is referred to as **CommonJSs** and `import { faker } from '@faker-js/faker'` syntax is referred to as **ECMAScript modules**.
...
If you so see something like this, you can convert it to the ECMAScript syntax by simply taking the variable declaration, `var { faker }` and replacing it with `import @faker-js/faker`.

Then you take the `= require(` from `var { faker } = require('@faker-js/faker');` and replace that with `from` and then get rid of the closing parenthesis.
...
![faker methods in console](./1285.png)
...
![generating a random firstName with faker and logging on console](./1286.png)
...
Let's try just importing the `person` subset from faker.

Modify the import like so 👇

```js
import { faker: { person } } from "@faker-js/faker";
```
...
```js
const fakeNames = Array.from({ length: 10 }, () => person.firstName());

console.log(fakeNames);
```

Faker also provides their own method to make this process more easy if you want to generate an array with a random length.

```js
const fakeNames = faker.helpers.multiple(() => person.firstName(), { count: { min: 5, max: 15 } });
...
Alternativly, you might want to use Faker directly for this job as well.

```js
const fakeNames = Array.from({ length: 10 }, () => person.fullName());
```

![generating an array of firstName and lastName with Faker and logging on console](./1287.png)
...
![documentation showing how we can calculate difference between two dates](./1289.png)
...
![formatDistance method showing difference in dates](./1290.png)
...
![logging the result of difference between dates in console](./1291.png)
...
![logging the result of difference between dates in console](./1292.png)
...
![formatting the current date with the help of date-fns docs](./1293.png)
...
![syntax of format function](./1294.png)
...
![different ways to format month](./1295.png)
...
![format the month from date](./1296.png)
...
![formatting the calendar year](./1297.png)
...
![formatting the year using date-fns in console](./1298.png)
...
![headers in the axios request](./1299.png)
...
![logging the response got from axios call](./1300.png)
...
![intersection of two arrays in console](./1302.png)
...
![comparing two object results in false in console](./1303.png)
...
![comparing two objects using isEqual using lodash results in true](./1304.png)
...
![await-to-js library in npm](./1305.png)
...
![executing checkIfNameIsCool function and getting cool name as log in console](./1306.png)
...
![uncaught error - bad name in console](./1307.png)
...
![catching the bad name error and logging in console](./1308.png)

As you can see, we get 2 things back: the error and undefined.

What if we switched it back to "wes" instead of snickers?

![switch the name back to wes instead of snickers and log the result in console](./1309.png)
...
![bad name error handled in console](./1310.png)
